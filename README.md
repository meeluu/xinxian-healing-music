...
- **Translation Pipeline M3 已完成**：引入 shared_preferences 本地持久化，ListeningSession 与 FeedbackRecord 保存到用户本地设备（最多 100 条），新增历史记录页支持查看 / 删除单条 / 清空全部。刷新页面或重新打开后记录保留，隐私模式下回退内存态。
- **本地存储说明**：所有聆听记录与反馈仅保存在本设备，不上传任何服务器，可在历史记录页随时删除。
- **当前仍使用 mock 实现**：情绪解析（mock 关键词规则）、音频生成（mock 本地素材）、音频后处理（直通）均为本地模拟，未接入真实 LLM / 音乐生成模型 / DSP 后处理。会话与反馈已本地持久化（shared_preferences），未接入云端数据库。后续替换 `mock_pipeline_factory` 的装配即可切换到真实链路，UI 代码无需改动。
...
4. 音频体验层：本地音频素材播放
5. 反馈采集层：本地状态表单
6. 会话记录层：ListeningSessionRecorder / FeedbackRepository Port，shared_preferences + JSON 本地持久化（最多 100 条），内存缓存 + 异步落盘，失败回退内存态。M2 同步方法保持不变，M3 新增 delete / clear。
...
本地持久化扩展点：M3 通过 shared_preferences + JSON 存储，ListeningSession / FeedbackRecord 各自独立 key（`xinxian.sessions` / `xinxian.feedback`），手写 toJson / fromJson，schemaVersion 字段支持未来迁移。云端数据库迁移时只需替换 `lib/pipeline/services.dart` 中的全局实现为 Remote 版本，Port 接口与 UI 代码无需改动。
...
- AI 音乐生成模型暂未接入
- DSP 音频后处理暂未真实执行
- 用户反馈与聆听会话已本地持久化（shared_preferences），最多保留最近 100 条；未接入云端数据库
- 隐私模式下本地持久化不可用时回退内存态，重启后丢失
