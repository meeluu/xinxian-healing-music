...
- **当前版本**：`v1.0.0 · P3-data-2 · Cloudflare Pages`
...
- **阶段**：`P3-Web-v1.0 进行中 / P3-data-2`
- **版本号**：`v1.0.0 · P3-data-2 · Cloudflare Pages`（首页底部显示 `心弦 v1.0.0 · P3-data-2 · Cloudflare Pages`）
- **构建日期**：2026-07-11
...
- 反馈数据分析与导出（`scripts/feedback-queries.sql` 16 条基础查询 + 附录 B 消融实验 6 条 + 测试数据隔离过滤 + Wrangler SQL + Cloudflare D1 Console）
- PowerShell 辅助查询脚本（`scripts/query-feedback.ps1`，支持 `-Recent` / `-ByTargetState` / `-ByAudio` / `-LowRating` / `-HighRating` / `-Notes` / `-Daily` / `-TextRatio` / `-ExcludeTest` / `-OnlyTest` 参数，P3 第一批 + 第二批新增）
