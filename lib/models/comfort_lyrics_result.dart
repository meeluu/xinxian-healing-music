/// 困惑解惑 + 歌词生成结果（P4 新方向第一批 / 第二批）。
///
/// 对应后端 `functions/api/comfort-lyrics.js` 的响应：
/// - [comfortInterpretation]：温和解惑文本（4 段：复述 / 重新框架 / 小行动 / 过渡到歌）
/// - [lyricDraft]：中文歌词草稿（含「主歌」「副歌」「尾声」标记 + 画面感 + 重复 hook）
/// - [songPrompt]：后续 AI 音乐生成的风格提示（英文，含 vocal/mood/tempo/instrumentation）
/// - [safetyNotes]：安全检查备注
/// - [source]：来源 'llm' / 'fallback' / 'mock'
/// - [scene]：场景识别（academic_failure / relationship_conflict / work_pressure / guilt_regret / default）
///
...
class ComfortLyricsResult {
  ...
  factory ComfortLyricsResult.fromJson(Map<String, dynamic> json) {
    return ComfortLyricsResult(
      comfortInterpretation: (json['comfortInterpretation'] as String?) ?? '',
      ...
      scene: (json['scene'] as String?) ?? 'default',
    );
  }
}
